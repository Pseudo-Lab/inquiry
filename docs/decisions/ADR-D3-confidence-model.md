...
- **[미해결·확정 전 필수] 평가 축별 산출 기준(rubric) 정의.** 5개 축(`plausibility`·`evidence_strength`·`novelty`·`impact`·`testability`) 각각에 대해 "무엇을 근거로 어떤 범위를 매기는가"를 judge-LLM 루브릭 수준으로 명확히 하지 않으면, 범위·출처를 요구해도 숫자 자체가 임의값이 되어 가짜 정밀도가 재발한다. 최소한 각 축의 정의문 + 앵커(0.0/0.5/1.0이 각각 무엇을 의미하는지)를 정해야 함. → D3 Accepted 전 해결 필요.
